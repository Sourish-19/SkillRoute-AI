...
import RevealLoader from './reveal-loader.tsx';
...
export const LandingPage: React.FC<LandingPageProps> = ({ onStart, showContent, showNav }) => {
      ...
      <div className="relative z-10">
        <div className="fixed top-0 left-0 w-full z-50 flex justify-center pointer-events-none">
          <motion.nav
            ...
            animate={{
              ...
              y: showNav ? (isScrolled ? 20 : 0) : -100,
              paddingLeft: isScrolled ? '1.5rem' : '2rem',
              paddingRight: isScrolled ? '1.5rem' : '2rem',
            }}
            transition={{ type: 'spring', damping: 25, stiffness: 200 }}
            className={`pointer-events-auto h-16 md:h-20 flex items-center justify-between border-b transition-all duration-500 overflow-hidden ${isScrolled
          ...
          >
            ...
            <div className="flex items-center gap-2 md:gap-4">
              <button
                onClick={onStart}
                className={`cursor-target text-sm font-medium transition-colors ${isScrolled ? 'text-zinc-400 hover:text-white px-2' : 'text-zinc-300 hover:text-white'
                  }`}
              >
                Log in
              </button>
        ...
        <section className="max-w-7xl mx-auto px-6 pt-48 pb-40 min-h-[90vh] flex items-center justify-center">
          <div className="text-center max-w-4xl mx-auto space-y-10 flex flex-col items-center">
            {showContent && (
              <BlurText
                text="✨ The Future of Career Guidance"
                delay={50}
                animateBy="words"
                direction="top"
            ...
            {showContent && (
              <BlurText
                text="Personalized learning, local job matching, and mentorship to launch your career, regardless of your location or infrastructure constraints."
                delay={30}
                animateBy="words"
                direction="top"
                className="text-xl text-zinc-400 leading-relaxed max-w-2xl mx-auto mt-6 text-center justify-center"
              />
            )}
